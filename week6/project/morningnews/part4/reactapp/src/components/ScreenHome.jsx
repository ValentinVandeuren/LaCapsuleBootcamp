...
function ScreenHome() {
  ...
  const [tokenSingIN, setTokenSingIN] = useState("");
  ...
  const submitSingIN = async () => {
    const signIn = await fetch('/sign-in', {
      method: 'POST',
      headers: {'Content-Type':'application/x-www-form-urlencoded'},
      body: `email=${emailSingIN}&password=${passwordSingIN}&token=${tokenSingIN}`
    })
    ...
    }else if(signInJSON.tokenFailedMessage) {
      setErrMessageSignIn(signInJSON.tokenFailedMessage)
  ...
  if(isSignUp || isSignIn){
    ...
    <div className="Login-page">
      ...
      <div className="Sign">
        ...
        className="Login-input"
        placeholder="Token"
        onChange={(e) => setTokenSingIN(e.target.value)}
        value={tokenSingIN}
        />
